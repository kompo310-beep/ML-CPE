"""Test the trained model on 4 random tomato leaf images (2x2 grid).
...
def test_nn(n_samples=N_SAMPLES):
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        # Tomato class names (e.g. "Tomato___Early_blight") are long, so use
        # a small font to keep the titles readable
        ax.set_title(f"Pred: {pred} ({confidence[i] * 100:.0f}%)\n"
                     f"True: {true}", color=color, fontsize=8)

        print(f"[{i + 1}] Pred: {pred:<40} True: {true:<40} "
