...
def plot_confusion_matrix(matrix, classes, save_path):
    ...
    ax.set_xticks(np.arange(len(classes)), classes, rotation=90)
...
def plot_kernel_comparison(accuracies, save_path):
    """Bar chart comparing accuracy across SVM kernels.

    accuracies: dict like {"linear": 0.81, "poly": 0.77, "rbf": 0.88}
    """

    kernels = list(accuracies.keys())
    scores = [accuracies[k] * 100 for k in kernels]

    fig, ax = plt.subplots(figsize=(5, 4))
    bars = ax.bar(kernels, scores, color=["#4C72B0", "#DD8452", "#55A868"])
